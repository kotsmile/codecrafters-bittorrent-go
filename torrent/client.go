...
func NewClient(meta *Meta, config *Config) *Client {
	return &Client{
		Meta:   meta,
		Config: config,
...
func (client *Client) RequestPeers(uploaded, downloaded, left, compact int) (*PeersResult, error) {
	...
	data, _, err := DecodeBencode(bencodeBody)
	if err != nil {
		return nil, err
	}
...
func (client *Client) DownloadPiece(peerAddress string, pieceIndex int) ([]byte, error) {
...
func (client *Client) ConnectAndGetDownloadPiece(peerAddress string, pieceIndex int) ([]byte, error) {
	...
	if _, err := client.Handshake(peerAddress); err != nil {
		return nil, err
	}
	...
	fmt.Printf("Downloading piece index: %d\n", pieceIndex)
	data, err := client.DownloadPiece(peerAddress, pieceIndex)
	if err != nil {
		return nil, err
	}
	fmt.Printf("Downloaded piece index: %d\n", pieceIndex)
	...
	if hex.EncodeToString(checkSum[:]) != pieces[pieceIndex] {
		return nil, errors.New("mismatched check sum")
	}
